...
(function () {
  ...
  var VIRTUAL_FILES = {
    greeting: ["def hello(name):", '    return "Hello, " + name + "!"'].join("\n"),
  };

  function compile(code) {
    const codeWithoutVariableTypeDefinitions = code.replace(
      /^ *(?!def\s)([\w.-]+) *: *[\w.-]+ *[:+*\/%&|^><-]*=/gm,
      "$1",
    );
  ...
  function on_input(ev) {
    ...
    if (code === 13 && !ev.ctrlKey && !ev.shiftKey && !ev.altKey && !ev.metaKey) {
      // Enter
      ...
      input.value = source + "\n" + next_line;
    } else if (code === 9 && !ev.ctrlKey && !ev.shiftKey && !ev.altKey && !ev.metaKey) {
      // Tab
      ev.preventDefault();
      check_for_completions();
    } else if (code === 27 && !ev.ctrlKey && !ev.shiftKey && !ev.altKey && !ev.metaKey) {
      // Esc
      hide_completions();
    } else if (code === 115) {
      // F4
